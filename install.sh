...
# عند التشغيل عبر 'curl | bash' يكون BASH_SOURCE[0] غير معرّف، ومع
# 'set -u' ينهار السكربت. نوفّر بديلاً آمناً ونعتمد على التنزيل من
# المستودع عند غياب الملفات محلياً.
...
if [ -t 0 ]; then
  read -r -p "هل تريد تشغيل الواجهة الآن؟ [Y/n] " LAUNCH || LAUNCH=""
  case "${LAUNCH,,}" in
